 ...
 */
// const getAbiProvider = async (account: any) => {
 ...
 */
export const EOSPubKeyToFIOPubKey = (eosPubKey: any) => {
 ...
 */
export const buyFIOHandle = async ({ fioPubKey, fioHandle }: any) => {
 const buyHandleResponse = await fetch(
  `${import.meta.env.VITE_SERVER_URL}/buyHandle`,
  {
   method: "POST",
   body: JSON.stringify({
